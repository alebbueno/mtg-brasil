...
// app/components/deck/DeckGridView.tsx
'use client'
...
// --- Tipo de Dados para este Componente ---
...
// --- Sub-componente para exibir uma única carta na grelha ---
function GridCardDisplay({ card, onCardHover, onCardLeave }: { card: GridCardData, onCardHover: (url: string) => void, onCardLeave: () => void }) {
  return (
    <div
      className="relative"
...
// --- Sub-componente ATUALIZADO para renderizar uma secção de cartas em GRELHA ---
function CardGridSection({ title, cards, onCardHover, onCardLeave, icon }: {
  title: string;
  cards: GridCardData[];
  onCardHover: (url: string) => void;
  onCardLeave: () => void;
    ...
    <div>
      ...
      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-8 gap-3">
        {cards
          ...
          .map((card) => (
            <GridCardDisplay key={card.id} card={card} onCardHover={onCardHover} onCardLeave={onCardLeave} />
          ))}
      </div>
    </div>
  );
...
// --- Componente Principal da Grelha ---
export default function DeckGridView({ commanderCard, planeswalkerCards, mainboardCards, sideboardCards, onCardHover, onCardLeave }: DeckGridViewProps) {
  
  // Agrupa as cartas do mainboard e sideboard por tipo
  const mainboardGrouped = useMemo(() => {
    return mainboardCards.reduce((acc, card) => {
      const mainType = card.type_line.includes("Creature") ? "Criaturas"
                      : card.type_line.includes("Land") ? "Terrenos"
                      : card.type_line.includes("Instant") ? "Mágicas Instantâneas"
                      : card.type_line.includes("Sorcery") ? "Feitiços"
                      : card.type_line.includes("Artifact") ? "Artefatos"
                      : card.type_line.includes("Enchantment") ? "Encantamentos"
                      : "Outros";
  ...
  const sideboardGrouped = useMemo(() => {
    return sideboardCards.reduce((acc, card) => {
      const mainType = card.type_line.includes("Creature") ? "Criaturas"
                      : card.type_line.includes("Land") ? "Terrenos"
                      : card.type_line.includes("Instant") ? "Mágicas Instantâneas"
                      : card.type_line.includes("Sorcery") ? "Feitiços"
  ...
  return (
    <div className="space-y-8">
      {/* Secção de Destaque para Comandante e Planeswalkers */}
                  ...
                  <div className="flex flex-wrap gap-4">
                      {planeswalkerCards.sort((a,b) => a.name.localeCompare(b.name)).map(card => (
                          <div key={card.id} className="w-[120px]">
                              <GridCardDisplay card={card} onCardHover={onCardHover} onCardLeave={onCardLeave} />
                          </div>
                      ))}
                  </div>
              </div>
      ...
      {/* Grelha para o Resto do Mainboard */}
