...
function BuildButton({ format, commanderName, hasPrompt }: {
  format: string;
  ...
  let isDisabled = pending || !format || (format !== 'commander' && !hasPrompt);
  if (!isDisabled && format === 'commander' && !commanderName && !hasPrompt) {
    isDisabled = true;
  }
...
export default function DeckBuildPage() {
    ...
    return (
      <div className="container mx-auto max-w-4xl py-8 animate-in fade-in">
        ...
        <div className="space-y-6">
          ...
          <form action={saveUserDeck} className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              <SaveButton />
              <Button variant="outline" type="button" size="lg" onClick={() => window.location.reload()}>Gerar Outro Deck</Button>
          </form>
        </div>
      </div>
    );
  ...
  // Senão, mostra o formulário inicial de geração
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 py-8 px-4 md:px-6">
      <div className="container mx-auto max-w-2xl">
        ...
        <form action={formAction}>
            <Card className="bg-neutral-900 border-neutral-800">
                        ...
                        <div className="space-y-2 animate-in fade-in">
                            ...
                            {!commander ? (
                                ...
                                <div className="flex items-center justify-between p-3 bg-neutral-800/50 border border-neutral-700 rounded-md">
                                    ...
                                    <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-neutral-400 hover:text-white" onClick={() => setCommander(null)}><X size={16} /></Button>
                                </div>
                            )}
                        </div>
                    )}
                ...
                <CardFooter className="flex-col items-start gap-4 pt-6">
                    ...
                    <BuildButton format={format} commanderName={commander?.name || null} hasPrompt={userPrompt.trim().length > 0} />
                </CardFooter>
            </Card>
            
            {/* Inputs ocultos para enviar dados do comandante para a action */}
        ...
        {state.error && !state.success && <p className="text-red-500 text-sm mt-4 text-center">{state.error}</p>}
      </div>
    </div>
  );
}